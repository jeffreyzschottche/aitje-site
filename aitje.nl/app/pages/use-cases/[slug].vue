  ...
  <div class="bg-[#f5f5f5] text-gray-900">
    ...
    <main v-if="item" class="px-6 pb-20 pt-32">
      <section class="mx-auto max-w-5xl">
        <NuxtLink to="/use-cases" class="text-sm font-semibold text-[#d4a700] transition hover:text-black">
      ...
      <section class="mx-auto mt-12 max-w-5xl">
        <article class="rounded-3xl border border-gray-200 bg-white p-8 shadow-sm sm:p-10">
          ...
          <h2 class="mt-3 text-3xl font-black text-gray-900">
            {{ item.contextTitle ?? "Waar deze use case over gaat" }}
          </h2>
        ...
        <div class="mt-6 grid gap-6 md:grid-cols-2">
          <article class="rounded-3xl border border-gray-200 bg-white p-8 shadow-sm">
            ...
            <h2 class="mt-3 text-2xl font-black text-gray-900">
              {{ item.problemTitle ?? "Wat loopt er vast?" }}
            </h2>
          ...
          <article class="rounded-3xl border border-gray-200 bg-white p-8 shadow-sm">
            ...
            <h2 class="mt-3 text-2xl font-black text-gray-900">
              {{ item.outcomeTitle ?? "Wat verandert er met AITJE?" }}
            </h2>
            <p class="mt-4 text-base leading-8 text-gray-600">
              {{ item.outcome }}
            </p>
        ...
        <article class="mt-6 rounded-3xl border border-gray-200 bg-white p-8 shadow-sm sm:p-10">
          <div class="max-w-3xl">
            ...
            <h2 class="mt-3 text-3xl font-black text-gray-900">
              {{ item.practicalTitle ?? "Wanneer dit goed past" }}
            </h2>
            <p class="mt-4 text-base leading-8 text-gray-600">
              {{ item.practical }}
            </p>
...
const item = computed(() => useCasesV2.nl.find((entry) => entry.slug === String(route.params.slug)));
